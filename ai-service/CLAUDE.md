...
- **품질 제약은 프롬프트가 아니라 코드로 강제한다.** 신뢰도 상한(`cap_confidence`), 제목·설명 길이와 금칙어(`copy_rules`)가 그 예다. 프롬프트 지시만으로 지켜지길 기대하지 않는다 — 지켜지지 않는다는 것이 측정으로 확인됐다.
- 모델에 판단을 시킬 때는 **그 판단의 근거를 별도 필드로 함께 받는다**(예: `identifiableReason`). 결과만 받으면 틀렸을 때 원인을 추측할 수밖에 없다.
